...
export const STARTER_PROMPTS = [
  {
    ...
    description: "读取文档、迁移模型配置、测通 CloudCLI 的 Claude Code",
...
要求：
1. 按文档检查和修复 agent.js 的模式配置。
2. 注意 OpenHouse 项目运行在 root，CloudCLI / Claude Code 的权限模式必须按文档处理。
3. DeepSeek 等 provider 可能同一个密钥对应不同协议，不能只复制 key；要确认 CloudCLI / Claude Code 实际支持的协议和 endpoint。
4. 测通目标是 CloudCLI 中的 Claude Code 网页侧调用，不是只测 Claude Code CLI。
5. 如果配置失败，或文档没有覆盖当前情况，请使用联网搜索工具检索最新方法后继续修复。
6. 配置成功后提醒我：通过 OpenHouse 菜单进入 cc/codex，也就是 CloudCLI / Claude Code 页面；默认账号密码如果仍是文档中的本机默认值，仅限本机使用，后续可以修改密码；进入后选择默认模型。`,
  },
  {
    title: "安装和配置 Hermes",
...
请先阅读并参考这些文档：
- ${OPENHOUSE_DOCS_DIR}/HERMES_SETUP.md
...
5. 为什么 pi-agent 主要负责首次配置和文档索引，主工作台可以由用户选择。
